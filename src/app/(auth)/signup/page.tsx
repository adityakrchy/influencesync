...
const App = () => {
  ...
  const handleInfluecerRegistration = async ()=>{
    try {
      const res = await axios.post("/api/auth/register", influencerData)
      console.log(res)
    } catch (error) {
  ...
  return (
    <div className="flex h-screen relative bg-gradient-to-r from-blue-600 to-indigo-700 overflow-hidden">
      ...
      <div
        id="leftSection"
        className={`text-white flex flex-col justify-center w-1/2 transition-all duration-500 p-8 ${activeSection === "right" ? "hidden" : "block"
          }`}
      >
        {activeSection === "left" ? (
      ...
      <div
        id="rightSection"
        className={`text-white w-1/2 flex flex-col justify-center items-center p-8 transition-all duration-500 ${activeSection === "left" ? "hidden" : "block"
          }`}
      >
        {activeSection === "right" ? (
